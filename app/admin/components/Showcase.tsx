...
function Showcase({ products, username }: ShowcaseProps) {
  ...
  // 📜 FINAL ROW MODE — after arc, cards settle into a horizontal slideable row
  const ROW_THRESHOLD = 2.2;
  const isRow = progress >= ROW_THRESHOLD;

  const visibleProducts = products.slice(0, 3);

  const renderCard = (product: ShowcaseType) => (
    <Card
      key={product.id}
      onClick={() => router.push(`/${username}/showcases/${product.id}`)}
      className="relative w-[70%] shrink-0 snap-center overflow-hidden rounded-[30] border-none transition-transform duration-300 ease-out hover:scale-[1.02]"
      style={{
  ...
  return (
    <div className="flex flex-col items-center mt-6 gap-4 w-full">
      ...
      {/* 📜 ROW MODE — horizontally slideable */}
      {isRow ? (
        <div
          ref={containerRef}
          className="w-full max-w-md flex items-stretch gap-4 overflow-x-auto pb-2 snap-x snap-mandatory scroll-smooth"
        >
